...
AnimatedStateSprite::AnimatedStateSprite(const char* name, const int frameCount, const int delay, const int stateCount)
		: Sprite(name)
{
...
void AnimatedStateSprite::draw(int x, int y, const double scale)
{
	...
	SDL_RenderCopy(renderer.get(), texture.get(), &clips[clipIndex], &dstRect);
	if (--currentDelay == 0)
	{
